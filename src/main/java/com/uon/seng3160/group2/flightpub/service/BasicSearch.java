...
public class BasicSearch implements FlightService {
    ...
    // get flights from path[u] to path[v] that are wihin range(startDate, maxDate)
    // for each flight
    // duplicate journey
    // add self to journey
    ...
    public List<List<Flight>> makeJourneys(List<Flight> journey, List<Destination> path, int u, int v, int minLayover,
        ...
        System.out.println(String.format("dest_u: %s, dest_v: %s, startTime: %s, upperTime: %s",
                path.get(u).getDestinationCode(), path.get(v).getDestinationCode(), startTime, upperTime));
    ...
    public List<Flight> realisePath(List<Destination> path, LocalDateTime departureTime) {
        ...
        System.out.println(String.format("start date: %s", dateFormatter.print(departureTime, Locale.ENGLISH)));
        System.out.println(String.format("end date: %s", dateFormatter.print(departureTimeUpper, Locale.ENGLISH)));

        Destination curr = path.get(0);
        Destination next = path.get(1);

        return flightRepository.findFlightsBetweenTimes(curr, next, departureTime, departureTimeUpper);
    }
    ...
    public List<List<Destination>> YensShortestPaths(Destination start, Destination end, int K) {
        ...
        PriorityQueue<List<Destination>> B = new PriorityQueue<>(Comparator.comparingInt(distB::get));
        ...
        for (int k = 1; k < K; k++) {
            ...
            for (int i = 0; i < spurRange; i++) {
                ...
                List<Destination> rootPath = A.get(k - 1).subList(0, i + 1); // may need to checK the (i+1), could be //
                                                                             // dangerous
